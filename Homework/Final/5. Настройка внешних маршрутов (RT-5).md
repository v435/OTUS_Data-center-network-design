...
![silent_host](attachments/rt5_silent_host2.drawio.png).

Пользователь отправляет ICMP-пинг на VTEP1 (свой шлюз), VTEP1 смотрит в таблицу маршрутизации и видит маршрут до сети назначения (192.168.200.0/24), под которую подпадает IP-адрес 192.168.200.20. Таким образом, пакет будет отправлен на VTEP2. VTEP2 увидит, что IP назначения принадлежит сети, которая напрямую к нему подключена в интерфейсе vlan 200, и со спокойной совестью сгенерирует ARP-запрос, чтобы узнать MAC-адрес "тихого узла", на который последний с радостью ответит. А вот когда он ответит, тут-то VTEP2 и анонсирует RT2 маршрут для "тихого" сервера :). Хотя, технически, он и не нужен - нам для маршрутизации вполне достаточно маршрута RT-5.
...
![RT5_Traffic_Walk](attachments/rt5_traffic_walk.drawio.png)

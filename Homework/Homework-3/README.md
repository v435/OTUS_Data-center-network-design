...
⚠️ __Примечание__ В рамках данного документа (выполнения работы) термин IS-IS следует считать равнозначным термину Integrated IS-IS.
...
2. Настроить процесс IS-IS на каждом Leaf и Spine, включить IS-IS на необходимых интерфейсах (стыки с соседями + loopback).
...
### Требования
* В рамках underlay-сети нашей топологии должна использоваться единая L2-зона.
* NET-адреса должны использовать AFI 49, номер зоны 1.
...
* Участники IS-IS должны аутентифицировать друг друга с использованием защищенного алгоритма sha256.
* Аутентификация должна быть настроена domain-wide (level-2).
...
Мы убедились, что стыковочные связи работают. Теперь начнем настраивать непосредственно процессы IS-IS и BFD на каждом устройстве.

#### Spine-1
Шаги настройки:
1) Мы инициализируем процесс IS-IS и задаем ему имя "OTUS".
2) Мы указываем NET-адрес как AFI 49 (Private LAN), Area ID 1, System ID 0100.0100.0001 (преобразовано из 10.1.0.1), Selector 00.
...
#### Spine-2
1) Мы инициализируем процесс IS-IS и задаем ему имя "OTUS".
2) Мы указываем NET-адрес как AFI 49 (Private LAN), Area ID 1, System ID 0100.0100.0002 (преобразовано из 10.1.0.2), Selector 00.
...
#### Leaf-1
1) Мы инициализируем процесс IS-IS и задаем ему имя "OTUS".
2) Мы указываем NET-адрес как AFI 49 (Private LAN), Area ID 1, System ID 0100.0100.1001 (преобразовано из 10.1.1.1), Selector 00.
...
#### Leaf-2
1) Мы инициализируем процесс IS-IS и задаем ему имя "OTUS".
2) Мы указываем NET-адрес как AFI 49 (Private LAN), Area ID 1, System ID 0100.0100.1002 (преобразовано из 10.1.1.2), Selector 00.
...
#### Leaf-3
1) Мы инициализируем процесс IS-IS и задаем ему имя "OTUS".
2) Мы указываем NET-адрес как AFI 49 (Private LAN), Area ID 1, System ID 0100.0100.1003 (преобразовано из 10.1.1.3), Selector 00.

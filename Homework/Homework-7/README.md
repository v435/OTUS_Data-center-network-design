...
Модель сервиса EVPN будет использована VLAN-Based.
...
### ESI
Идентификаторы Multihomed-сегментов и Route-Target'ы для маршрутов RT-4.

| Client   | ESI                      | ES-Import-RT      |
| -------- | ------------------------ | ----------------- |
| Client-1 | 0000:0000:0000:0000:0001 | 00:00:00:00:00:01 |
| Client-2 | 0000:0000:0000:0000:0002 | 00:00:00:00:00:02 |
| Client-3 | 0000:0000:0000:0000:0003 | 00:00:00:00:00:03 |
| Client-4 | 0000:0000:0000:0000:0004 | 00:00:00:00:00:04 | 
...
Создадим LAG-интерфейсы, которые будут смотреть в сторону клиентов. На каждого из клиентов на этом лифе будет одна нога (вторая будет на соседнем Leaf'е - ведь у нас ESI LAG).
```
...
```
Помимо перевода интерфейса в режим VLAN 10 Untagged, мы здесь задаем MAC-адрес LACP-канала. Он должен быть одинаковым на обоих Leaf'ах-ногах Port-Channel'а, чтобы клиент считал оба линка ведущими к одному устройству (он ведь ничего не знает ни про какие ESI LAG, для него канал выглядит два линка к одному физическому устройству).

Также в этой секции мы задаем две важные настройки ESI: идентификатор сегмента и Route-target сегмента. Идентификатор сегмента будет уникально идентифицировать этот линк как часть некоего набора линков, ведущих к одной и той же клиентской multihomed-системе. Благодаря идентификатору сегмента VTEP'ы будут четко знать, кто еще из своих пиров смотрит в этот же сегмент. На основе этого идентификатора будет сгенерирован маршрут RT-1 (Auto-Discovery), который будет использоваться VTEP'ами для обнаружения соседей, смотрящих в этот же VTEP, для предотвращения закольцовывания BUM-трафика и для ECMP - неважно, с какого ESI-VTEP'а был получен RT-2 маршрут: другие VTEP'ы будут знать, что трафик до конкретной системы в данном ESI можно отправить на любой VTEP, имеющий подключение к этому же Ethernet Segment'у. Это называется механизм aliasing.

Также в этой секции мы задаем ES Import Route-Target. Задание этого комьюнити требуется для генерации маршрутов RT-4 (Ethernet Segment Route). Благодаря этому комьюнити VTEP'ы, подключенные к одной и той же multihomed-системе смогут импортировать маршруты RT-4 для этого сегмента. Вообще по идее значение должно генерироваться автоматически на основе ESI, но у меня на Аристе почему-то без явного указания RT маршруты RT-4 отказывались генерироваться. Вообще маршрут RT-4 нужен для того, чтобы VTEP'ы, подключенные к одному ES, смогли выбрать Designated Forwarder'а, то есть VTEP, которому будет разрешено пересылать BUM-трафик в ES-сегмент (нужно для предотвращения возникновения петель в ES-сегменту). 

На этом настройки, связанные непосредственно с Multihoming, закончены :). Идем дальше. У нас еще поднятие третьего уровня и пара мелочей.
...
```
mode active означает, что мы будем активно пытаться установить LACP-соседство вместо того, чтобы пассивно ожидать, пока нас позовут его оформить с другого конца линка :).

IRB-интерфейс, нужно для третьего уровня. Задаем IP-адрес шлюза по умолчанию (он же Static Anycast Gateway). Этот интерфейс назначаем в наш VRF1:
```
interface Vlan10
   vrf VRF1
...
Настраиваем VXLAN. Указываем, откуда брать Source IP для VXLAN-пакетов (с Loopback'а), задаем соответствие vlan 10 и vni 1010. Создаем L3VNI путем назначения VRF1 "виэнайки" 1000.
...
Зададим anycast-MAC для наших IRB-интерфейсов:
```
...
Пропишем для нашего MAC-VRF для VLAN 10 соответствующий RD и RT (RD, точнее, не пропишем, а скажем генерировать его автоматически). Включим распространение информации в EVPN (RT-2) о локально подключенных хостах в VLAN 10 с помощью команды redistribute learned.
```
router bgp 65001
   !
   vlan 10
...
Активируем AFI/SAFI EVPN для нашего MP-BGP:
```
router bgp 65001
   address-family evpn
      neighbor CLOS activate
...
Приведу еще настройки клиентов. Тут ничего необычного - виртуалки на основе Ubuntu Server, настройки выполнены через Netplan, создан LACP LAG на двух интерфейсах, смотрящих в разные VTEP'ы.
...
<!-- TOC --><a name="-5"></a>
## Верификация
Предлагаю не проверять сейчас каждую мелочь (L2/L3 были проверены в предыдущих ДЗ). Сейчас верифицируем только основной статус BGP (соседство), проверим маршруты, разберем, все ли в порядке с нашими ESI, а потом проверим пингами доступность всех клиентов друг для друга. И напоследок - погасим по очереди один и другой линк на одном из клиентов и проверить, будет ли отрабатывать переход трафика с упавшего линка на уцелевший.


С учетом того, что Spine с точки зрения MH нас особо не интересуют, ограничимся только самыми общими проверками.
...
Вот тут посмотрим поподробнее. VTEP'ы должны сгенерировать по одному маршруту RT-1 для каждого EVI И еще один RT-1 для Ethernet-сегмента (без привязки к EVI).
 ...
 Paths: 2 available
  ...
  65100 65003
    10.1.1.3 from fe80::5290:ceff:fec1:219%Et2 (10.1.0.2)
      ...
      VNI: 0
```
Как я подозреваю, маршруты с VNI: 0 - это маршруты, сгенерированные Per-ESI, то есть общие для сегмента. Я, если честно, забыл, для чего генерировать маршрут RT-1 per-ESI, но, по-моему, это нужно для Mass Withdrawal. Если у нас падает линк в сегмент, мы отзываем данный маршрут, и все другие VTEP'ы понимают, что любые EVI, доступные через эти сегменты, становятся недоступными, и трафик слать через этот VTEP в них нельзя. Могу ошибаться, но по логике должно быть так.
 ...
 Paths: 2 available
  ...
  65100 65003
    10.1.1.3 from fe80::5290:ceff:fec1:219%Et2 (10.1.0.2)
      ...
      Extended Community: TunnelEncap:tunnelTypeVxlan EvpnEsImportRt:00:00:00:00:00:04
```
Так, ну тут мы видим ES-Import-RT, которое мы задавали в конфигурации, для нужных ESI. А вот что я тут не вижу, это указание на то, кто сейчас является DF'ом. Ладно, посмотрим потом в show-командах. Маршруты для всех ESI у нас присутствуют, так что здесь тоже все в порядке.
 ...
 Paths: 2 available
  ...
  65100 65003
    10.1.1.3 from fe80::5290:ceff:fec1:219%Et2 (10.1.0.2)
      ...
      VNI: 1020 L3 VNI: 1000 ESI: 0000:0000:0000:0000:0004
```
Подробный вывод для того, чтобы убедиться, что хосты у нас видны в своих ESI. И это так и есть - ESI у них ненулевой, а тот, которому принадлежат линки, которыми хосты подключены к нашим VTEP'ам. Что нам и нужно было увидеть.
...
EVPN instance: VLAN 10
  ...
  Local ethernet segment:
    ...
    ESI: 0000:0000:0000:0000:0001
      ...
      Non-Designated forwarder: 10.1.1.2
```
Что нам тут важно увидеть: 1) локальные Ethernet-сегменты и кто является для них Designated Forwader'ом, а кто не является таковым. Также можем убедиться, что у нас включен режим All-Active (что логично, у нас же LACP LAG в сторону клиентов), оба линка к клиенту могут использоваться одновременно.
...
Проверять будем таким образом. Сначала мы на клиенте делаем пинг всех других клиентов (проверим таким образом intra-VNI и inter-VNI доступность).
После этого мы запустим пинг с Client-1 до Client-4, а на Client-4 попеременно отключим обе ноги и проверим, что трафик перетекает на вторую целую ногу. Учитывая, что у меня здесь виртуалки и ресурсов мало, будет заметный лаг между переключениями с линка на линк. :-/
